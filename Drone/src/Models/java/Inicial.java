...
public class Inicial extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    // Output ports
    //ID:OUTP:0
    public final Port<Serializable> outComando =
        addOutputPort("outComando", Serializable.class);
    ...
    public Inicial(String name) {
        this(name, null);
    }

    public Inicial(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        Inicial model = new Inicial();
        model.options = options;

        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("Inicial Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = Inicial.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                Inicial.class.getResource(".").toString());
        }
